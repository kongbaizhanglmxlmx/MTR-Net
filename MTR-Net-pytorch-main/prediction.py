...
class Predictor:
    ...
    def get_score(self, values):
        ...
        data = SlidingWindowDataset(values, self.window_size, self.target_dims, labels=None)
    ...
    def predict_anomalies(self, train, test, true_anomalies, load_scores=False, save_output=True,
        ...
        if load_scores:
            ...
            test_pred_df = pd.read_pickle(
                f"{self.save_path}/test_output.pkl"
            )

            train_anomaly_scores = (
                train_pred_df["A_Score_Global"].values
            )
            test_anomaly_scores = (
                test_pred_df["A_Score_Global"].values
            )
        ...
        # Only keep the required evaluation metrics
        epsilon_result = {
            "f1": float(e_eval["f1"]),
            "precision": float(e_eval["precision"]),
            "recall": float(e_eval["recall"]),
            "TP": float(e_eval["TP"]),
            "TN": float(e_eval["TN"]),
            "FP": float(e_eval["FP"]),
            "FN": float(e_eval["FN"]),
            "threshold": float(e_eval["threshold"]),
        }
        ...
        with open(
                f"{self.save_path}/{self.summary_file_name}",
                "w",
                encoding="utf-8"
        ) as f:
            json.dump(
        ...
        if save_output:
            global_epsilon = epsilon_result["threshold"]
